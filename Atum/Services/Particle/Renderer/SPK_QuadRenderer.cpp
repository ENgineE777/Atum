...
namespace SPK
{
	...
	void ParticleProgram::ApplyStates()
	{
		...
		core.render.GetDevice()->SetBlendFunc(Device::ArgSrcAlpha, Device::ArgOne);
		core.render.GetDevice()->SetCulling(Device::CullNone);
	};
	...
	GLQuadRenderer::GLQuadRenderer(float scaleX,float scaleY) :
		...
		textureIndex(0)
	{
		VertexDecl::ElemDesc desc[] = { { VertexDecl::Float3, VertexDecl::Position, 0 },{ VertexDecl::Float2, VertexDecl::Texcoord, 0 },{ VertexDecl::Ubyte4, VertexDecl::Color, 0 } };
		vdecl = core.render.GetDevice()->CreateVertexDecl(3, desc);
		prg = core.render.GetProgram("ParticleProgram");
	}
	...
	void GLQuadRenderer::render(const Group& group,const DataSet* dataSet,RenderBuffer* renderBuffer) const
	{
		...
		Matrix view;
		core.render.GetTransform(Render::Transform::View, view);
		//view.Transpose();
