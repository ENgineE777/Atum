...
namespace SPK
{
	...
	void Collider::modify(Group& group,DataSet* dataSet,float deltaTime) const
	{
		...
		for (GroupIterator particleIt0(group); !particleIt0.end(); ++particleIt0)
		{
			...
			int index0 = particle0.getIndex();

			const Octree::Array<int>& neighborCells = octree.getNeighborCells(index0);
			int nbCells = neighborCells.size();

			for (int i = 0; i < nbCells; ++i) // For each neighboring cell in the octree
			{
				const Octree::Cell& cell = octree.getCell(neighborCells[i]);
				int nbParticleInCells = cell.particles.size();

				for (int j = 0; j < nbParticleInCells; ++j) // for each particles in the cell
				{
					int index1 = cell.particles[j];
					if (index1 >= index0)
