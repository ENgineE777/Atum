...
namespace SPK
{
	...
	*/
	...
	private :
		...
		int updateTankFromTime(float deltaTime);
		int updateTankFromNb(int nb);
	...
	inline int Emitter::updateTankFromTime(float deltaTime)
	{
		if (deltaTime < 0.0f)
			return 0;
	...
	inline int  Emitter::updateTankFromNb(int nb)
	{
		if (currentTank < 0)
			return nb;
