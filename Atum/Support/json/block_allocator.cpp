...
block_allocator::block_allocator(int blocksize): m_head(0), m_blocksize(blocksize)
{
}
...
void *block_allocator::malloc(int size)
{
	if ((m_head && m_head->used + size > m_head->size) || !m_head)
	{
		// calc needed size for allocation
		int alloc_size = sizeof(block) + size;
		
		if (alloc_size < m_blocksize)
		{
			alloc_size = m_blocksize;			
		}
