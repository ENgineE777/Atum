...
namespace SPK
{
	...
	const int Octree::MAX_LEVEL_INDEX = 4; // A too high number will be highly memory comsuming
	const int Octree::MAX_PARTICLES_NB_PER_CELL = 32;
	...
	void Octree::update()
	{
		...
		if (group.getCapacity() != nbParticles)
		{
			...
			particleCells = SPK_NEW_ARRAY(Array<int>,nbParticles);
		...
		for (ConstGroupIterator particleIt(group); !particleIt.end(); ++particleIt)
		{
			...
			int particleIndex = particle.getIndex();
		...
		for (int i = 0; i < nbCells; ++i)
			...
			{
				activeCells.push(i);
				int nbParticlesInCells = cells[i].particles.size();
				for (int j = 0; j < nbParticlesInCells; ++j)
					particleCells[cells[i].particles[j]].push(i);
			}
	}

	int Octree::initNextCell(int level, int offsetX, int offsetY, int offsetZ)
	{
	...
	void Octree::addToCell(int cellIndex, int particleIndex, int maxLevel)
	{
		...
		else
		{
			...
			if (!cell.hasChildren)
			{
				...
				int nbParticlesInCell = cells[cellIndex].particles.size();
				for (int i = 0; i < nbParticlesInCell; ++i)
	...
	void Octree::addToChildrenCells(int parentIndex, int particleIndex, int maxLevel) // TODO This code may be optimized ?
	{
		const Cell& parent = cells[parentIndex];
		int childLevel = parent.level + 1;
		int divisor = maxLevel - childLevel;
