...
void NavMesh2D::BindClassToScript()
{
	...
	BIND_TYPE_TO_SCRIPT(NavMesh2D, brief)
	...
	core.scripts.RegisterObjectMethod(script_class_name, "int GetPath(float x, float y, float to_x, float to_y)", WRAP_MFN(NavMesh2D, GetPath), brief);

	brief = "Remove a path\n"
		"\n"
		"\\param[in] index Index of a path\n";

	core.scripts.RegisterObjectMethod(script_class_name, "void RemovePath(int index)", WRAP_MFN(NavMesh2D, RemovePath), brief);
	...
	core.scripts.RegisterObjectMethod(script_class_name, "int MoveAlongPath(int index, float dist)", WRAP_MFN(NavMesh2D, MoveAlongPath), brief);
	...
	core.scripts.RegisterObjectMethod(script_class_name, "void GetPathCurPoint(int index, float&out pos_x, float&out pos_y, float&out angle)", WRAP_MFN(NavMesh2D, GetPathCurPoint), brief);
}
